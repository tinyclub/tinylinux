...
# Search the param setup by __raw_setup at first, then __raw_early_param

for d in "init kernel arch/$ARCH mm block ipc crypto net lib security virt sound drivers fs"
do
	for f in `find $srctree/$d -name "*.c"`
	do
		# Is there a __raw_setup("param?
		param_type="__raw_setup"
		ret=`grep -m 1 -l $param_type\(\"$param $f`
		if [ -n "$ret" ]; then
			file=$ret
			target_param_type="__setup"
			break;
		else
			# Is there a __raw_early_param("param
			param_type="__raw_early_param"
			ret=`grep -m 1 -l $param_type\(\"$param $f`
			if [ -n "$ret" ]; then
				file=$ret
				target_param_type="early_param"
				break;
			fi
		fi
	done
...
# Prompt
echo "The param $param is unsetup with the empty $target_param_type in $file"
